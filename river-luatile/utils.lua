...
M.height_for_n = function(args, n)
    ...
    return (args.height - (args.inner_gaps * (n - 1)) - (args.outer_gaps * 2)) / n
end
...
M.y_of_i = function(args, n, i)
    ...
    return args.outer_gaps + args.inner_gaps * (i - 1) + M.height_for_n(args, n) * (i - 1)
end

--- given n windows on a row, return the width of each window
M.width_for_n = function(args, n)
    -- return (args.width - GAPS * (n + 1)) / n
    return (args.width - (args.inner_gaps * (n - 1)) - (args.outer_gaps * 2)) / n
end
...
M.x_of_i = function(args, n, i)
    ...
    return args.outer_gaps + args.inner_gaps * (i - 1) + M.width_for_n(args, n) * (i - 1)
end
